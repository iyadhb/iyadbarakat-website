...
Then, when you want the full system — all six frameworks laid out, with the exact templates — I've made them free. You can [get the six frameworks here](/#newsletter). And if you'd rather skip straight to a hundred ready-to-run prompts built on them, each with its framework, template and adaptation notes, that's [the Vault](/#toolkit).
